...
void DescriptorHeap::create(ID3D12Device * device, uint32 maxDescriptorCount) {
...
BufferView * DescriptorHeap::allocateBufferView(uint32 descriptorCount) {
...
void DescriptorHeap::discardBufferView(BufferView * bufferView) {
...
DescriptorHeapManager::DescriptorHeapManager() {
	_rtvHeap = new DescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
	_dsvHeap = new DescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
	_cbvSrvHeap = new DescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
}
...
void DescriptorHeapManager::create(ID3D12Device * device) {
...
void DescriptorHeapManager::createRenderTargetView(ID3D12Resource ** textureBuffers, BufferView ** dstView, uint32 viewCount) {
	...
	BufferView* rtv = _rtvHeap->allocateBufferView(viewCount);
...
void DescriptorHeapManager::createConstantBufferView(ID3D12Resource ** constantBuffers, BufferView ** dstView, uint32 viewCount) {
	...
	BufferView* cbv = _cbvSrvHeap->allocateBufferView(viewCount);
